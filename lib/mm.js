...
import { ADD, SUBTRACT, MULTIPLY, DIVIDE, DIFFICULTIES } from "../config.js";
import {
  randBetween,
  smallestInt,
  largestInt,
...
const printGranularStats = async () => {
...
const gracefulExit = async (summary) => {
  ...
  console.log(
    `THE END. ${correct}/${num_probs} correct. ${(
      (correct / num_probs) *
      100
    ).toFixed(1)}%`
  );
  await printGranularStats();
  await printAggregateStats();
  process.exit(0);
};
...
export const repl = async () => {
  ...
  const difficulty = parseInt(
    await rl.question(
      "Enter difficulty level between 1 (easy) and 10 (hard):\n"
    )
  );
  if (difficulty <= 1 || difficulty >= 10)
    exitWithError(`Invalid difficulty: ${difficulty}`);

  let user_ops = await rl.question(
    "What types of operations [+, -, *, /]? (Enter comma separated list)\n"
  );
